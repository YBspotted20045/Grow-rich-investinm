...
export default function Signup(){
  ...
  const handleSubmit = async (ev) => {
    ...
    setTimeout(()=>{
      setLoading(false);
      alert("Signup success (demo) — please login");
    }, 2000);
  };
  ...
  return (
    <div className="lux-signup-page">
      ...
      <div className="moving-logos">
        {[...Array(30)].map((_, i) => (
          <img key={i} src="/logo.png" alt="mini" className="mini-logo" style={{
            left: `${Math.random()*100}%`,
            top: `${Math.random()*100}%`,
      ...
      <div className="form-wrap">
        <form className="lux-form" onSubmit={handleSubmit}>
          ...
          <input name="fullName" value={data.fullName} onChange={e=>setData({...data, fullName:e.target.value})} disabled={loading}/ >
          {errors.fullName && <div className="form-error">{errors.fullName}</div>}

          <label>State</label>
          <select name="state" value={data.state} onChange={e=>setData({...data, state:e.target.value})} disabled={loading}>
          ...
          <label>Age</label>
          <input type="number" name="age" value={data.age} onChange={e=>setData({...data, age:e.target.value})} disabled={loading}/>
          {errors.age && <div className="form-error">{errors.age}</div>}

          <label>Email</label>
          <input type="email" name="email" value={data.email} onChange={e=>setData({...data, email:e.target.value})} disabled={loading}/>
          {errors.email && <div className="form-error">{errors.email}</div>}

          <label>Password</label>
          <input type="password" name="password" value={data.password} onChange={e=>setData({...data, password:e.target.value})} disabled={loading}/>
          {errors.password && <div className="form-error">{errors.password}</div>}

          <label>Confirm Password</label>
          <input type="password" name="confirm" value={data.confirm} onChange={e=>setData({...data, confirm:e.target.value})} disabled={loading}/>
